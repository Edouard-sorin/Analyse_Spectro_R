...
model_SVM_36 <- svm(  y = train_svm[[paste0("qPCR_", seuil.ct)]]       #   on fix la valeur de ct à predir    
...
svm_pred$svm_pred_36 <- predict(model_SVM_36 ,newdata = test_svm[, grep("^X", names(test_svm))] , decision.values = T)
...
svm_pred_arbres_36$statut_pred <- "Indéterminé"
svm_pred_arbres_36$statut_pred[svm_pred_arbres_36$svm_pred_36 >= crit.pos  ] <- "Positif"
svm_pred_arbres_36$statut_pred[svm_pred_arbres_36$svm_pred_36 <= crit.neg  ] <- "Négatif"
...
TP <- svm_confusion_matrix_36[1,1]

TN <- svm_confusion_matrix_36[2,2]

FN <- svm_confusion_matrix_36[2,1]
...
Accuracy_svm36 <- ((TP+TN) / (TP+TN+FN+FP)*100)

Precision_svm36 <- ((TP / (TP+FP)*100))

Sensitivity_svm36 <- ((TP / (TP+FN)*100))

Parametre_svm_36 <- rbind(Accuracy_svm36,Precision_svm36,Sensitivity_svm36)

svm_pred_arbres_36$Vrai = "FAUX"
svm_pred_arbres_36$Vrai[svm_pred_arbres_36$code_ech_arbre %in% trueP$seuil.36] = "VRAI"

#save(svm_confusion_matrix_36,Accuracy_svm36,Precision_svm36,Sensitivity_svm36,svm_pred_arbres_36,Parametre_svm_36,  file = "Sauvegardes_objet_R.data/Pred_svm/parameter_svm_ct36.Rdata")
...
svm_pred <- ggplot(svm_pred_arbres_36)+
