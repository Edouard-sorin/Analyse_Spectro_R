...
rf_pred$rf_pred_32 <- predict(model_rf_32,newdata = test_rf, decision.values = T)
...
rf_pred_arbres_32$statut_pred <- "Indéterminé"
rf_pred_arbres_32$statut_pred[rf_pred_arbres_32$rf_pred_32 >= crit.pos  ] <- "Positif"
rf_pred_arbres_32$statut_pred[rf_pred_arbres_32$rf_pred_32 <= crit.neg  ] <- "Négatif"
...
TP <- rf_confusion_matrix_32[1,1]

TN <- rf_confusion_matrix_32[2,2]

FN <- rf_confusion_matrix_32[2,1]

FP <- rf_confusion_matrix_32[1,2]

rf_confusion_matrix_32

# Calcul des parametres rf de la matrice de confusion

Accuracy_rf32 <- ((TP+TN) / (TP+TN+FN+FP)*100)

Precision_rf32 <- ((TP / (TP+FP)*100))

Sensitivity_rf32 <- ((TP / (TP+FN)*100))

Parametre_rf_32 <- rbind(Accuracy_rf32,Precision_rf32,Sensitivity_rf32)

rf_pred_arbres_32$Vrai = "FAUX"
rf_pred_arbres_32$Vrai[rf_pred_arbres_32$code_ech_arbre %in% trueP$seuil.32] = "VRAI"

#save(rf_confusion_matrix_32,Accuracy_rf32,Precision_rf32,Sensitivity_rf32,rf_pred_arbres_32,Parametre_rf_32,  file = "Sauvegardes_objet_R.data/Pred_RF/parameter_RF_ct32_1000trees.Rdata")
...
rf_pred <- ggplot(rf_pred_arbres_32)+
  ...
  labs(x = "Code de l'arbre", y = "Résultat moyen des feuilles entre 0 et 1", title = "Prédiction svm rf Global a ct32  " ,subtitle = "T = positif au HLB à Ct32 en qPCR", color = "Statut") +dark_theme_gray()

rf_pred

#ggsave("Graphiques/Graph_RF/Prediction_rf_Global_ct32.pdf",plot = last_plot(), units = "cm", width = 20, height = 15, scale = 2)
