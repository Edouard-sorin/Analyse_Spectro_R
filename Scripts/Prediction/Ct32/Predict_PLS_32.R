...
plot( model_pls_32,main="Test Dataset", xlab="observed", ylab="PLS Predicted")
...
pls_pred$pls_pred_32 <- predict(model_pls_32, newdata = test_pls[, grep("^X", names(test_pls))], decision.values = T, ncomp=100)
...
pls_pred_arbres_32$statut_pred <- "Indéterminé"
pls_pred_arbres_32$statut_pred[pls_pred_arbres_32$pls_pred_32 >= crit.pos  ] <- "Positif"
pls_pred_arbres_32$statut_pred[pls_pred_arbres_32$pls_pred_32 <= crit.neg  ] <- "Négatif"
...
TP <- pls_confusion_matrix_32[1,1]

TN <- pls_confusion_matrix_32[2,2]

FN <- pls_confusion_matrix_32[2,1]

FP <- pls_confusion_matrix_32[1,2]

pls_confusion_matrix_32

# Calcul des parametres pls de la matrice de confusion

Accuracy_pls32 <- ((TP+TN) / (TP+TN+FN+FP)*100)

Precision_pls32 <- ((TP / (TP+FP)*100))

Sensitivity_pls32 <- ((TP / (TP+FN)*100))

Parametre_pls_32 <- rbind(Accuracy_pls32,Precision_pls32,Sensitivity_pls32)

pls_pred_arbres_32$Vrai = "FAUX"
pls_pred_arbres_32$Vrai[pls_pred_arbres_32$code_ech_arbre %in% trueP$seuil.32] = "VRAI"

#save(pls_confusion_matrix_32,Accuracy_pls32,Precision_pls32,Sensitivity_pls32,pls_pred_arbres_32,Parametre_pls_32,  file = "Sauvegardes_objet_R.data/Pred_pls/parameter_pls_ct32.Rdata")
...
pls_pred <- ggplot(pls_pred_arbres_32)+
  aes(y = pls_pred_32 , x = code_ech_arbre, color = statut_pred) +
  ...
  geom_segment(aes(xend = code_ech_arbre, y = 0, yend = pls_pred_32)) +
  scale_color_manual(values = c(Négatif = "red", Indéterminé = "gray20", Positif = "green")) +
  labs(x = "Code de l'arbre", y = "Résultat moyen des feuilles entre 0 et 1", title = "Prédiction pls Global a ct32  " ,subtitle = "T = positif au HLB à Ct32 en qPCR", color = "Statut") +dark_theme_gray()

pls_pred

#ggsave("Graphiques/Graph_pls/Prediction_pls_Global_ct32.pdf",plot = last_plot(), units = "cm", width = 20, height = 15, scale = 2)

#ggsave("Graphiques/Graph_pred_global_32.pdf",plot = last_plot(), units = "cm", width = 20, height = 15, scale = 2)

#save(All_parametre_32,All_Confusion_matrix_32 , file = "Sauvegardes_objet_R.data/All_parametre_32.Rdata")

# write.table(x = All_Confusion_matrix_32  , file = "Donnees/All_Confusion_matrix_32 .csv" , sep = ';')
