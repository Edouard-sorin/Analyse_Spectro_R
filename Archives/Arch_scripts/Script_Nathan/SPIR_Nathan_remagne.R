...
# Obtenir les mêmes résultats que Natahan en comprenant la démarche

# Connaitre la précision du résultat vis à vis du nombre d'échantillonngages


# -> Données de Nathan ####

# Lire les données de Nathan
...
#mypath_var_pheno="Données/Données_Nathan/var_pheno/var_pheno_xlsx" #select the path where your result data files are

mypath_var_pheno <- read.csv(file ="Donnees/Donnees_Nathan/var_pheno", sep=";") # march pas
...
# -> Mise en forme du jeu de données test_nb_rep_spir ####
...
spir <- read.csv(file ="Donnees/Donnees_Nathan//spir_rep_trees_1_2.csv", sep=";")
obs_N1 <- as.data.frame(read_excel("Donnees/Donnees_Nathan/spir arbres 1-2.xlsx", sheet = "N1")[,2:8])
obs_N2 <- as.data.frame(read_excel("Donnees/Donnees_Nathan/spir arbres 1-2.xlsx", sheet = "N2")[,2:8])
obs_P1 <- as.data.frame(read_excel("Donnees/Donnees_Nathan/spir arbres 1-2.xlsx", sheet = "P1")[,2:8])
obs_P2 <- as.data.frame(read_excel("Donnees/Donnees_Nathan/spir arbres 1-2.xlsx", sheet = "P2")[,2:8])

obs_N1 <- obs_N1[-31:-59,] # pour enlever les lignes de NA de obs_N1

obs <- list(N1=obs_N1[1:30,], N2=obs_N2[1:30,], P1=obs_P1[1:30,], P2=obs_P2[1:30,]) # TTe les obs ss forme de liste
obs <- lapply(X = obs, FUN = function(x){replace(x, is.na(x), 0)}) # on convertit les NA en 0

# creation de la data.frame obs_global à partir de la list obs cree juste avant
...
# Mettre la data.frame en facteur

for (i in 1:ncol(obs_global)) {
	obs_global[,i] <- factor(obs_global[,i])
}

ftable (rep_lot ~ HLB , data = obs_global) # repartition equitable des lots

ftable (rep_feuille ~ HLB , data = obs_global) # repartition equitable des feuilles analysés

# creation de nouvelles colonnes

for (i in 1:nrow(spir)) {
	code_labo <- spir$X[i]                   # X devient temporairement code_labo
...
ftable (rep_lot ~ HLB , data = test)  # repartition inequitable des lots

ftable (rep_feuille ~ HLB , data = test) # repartition inequitable des feuilles

ftable (rep_feuille + rep_lot ~ HLB + rep_arbre , data = test) # exploration du jeu de données

for (i in 1:8) {
	test[,i] <- factor(test[,i]) # on convertit les 8 premières col d'obs en facteur
}

test <- na.omit(test) # on passe de 1191 lignes à 1181  ou sont les NA ?

spir <- test[,-c(8,9)]  # pour enlever les ID et la colonne X_1 qui est inutile , pour ça que spir à 2158 Variables et test en à 2160

spir$HLB <- mapvalues(spir$HLB, from=c("N", "P"), to=c(0,1))  # transformer les N et P en 0 et 1
class(spir)
...
tirage <- do.call(rbind, lapply(1:nrow(spir), function(x) {
	data.frame(tirage= paste(unlist(spir[x,1:4]), collapse = ""))  # tirage correspond à l'ensemble (HLB + rep_arbre + rep_lot + rep_feuille) combiné en 1 seul colonne
}))
...
rm(list= c("obs_N1","obs_N2","obs_P1","obs_P2","obs")) # supression des listes inutiles
...
# ggsave(filename = "Graphiques/Parametres_SVM_SPIR_Nathan.png"
#        , plot = last_plot())
...
#write.csv(data_global, "datarep_2.csv")
#data_global_rep$rep <- rep(seq(1:10), each=10)


#save(list = ls(), file = "Analyse_Spectro_R.Rproj")

#Voir sur site https://mtes-mct.github.io/parcours-r/m1/sauvegarder-son-travail.html

#Data_Nathan <- read.csv(file ="Sauvegardes_objet_R.data/datarep.csv", sep=",")
# View(Data_Nathan)

 #save(list = ls(), file = "outputs")  Sauvegarder tt les data_frame
