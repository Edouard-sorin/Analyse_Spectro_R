...
library(snowfall) # utilisation paralell
...
# Importation des donnees data_SPIR_Ed avec variete ####

data_SPIR_Ed <- read.table(file = "donnees/SPIR_Hoarau_var.csv"
                           , header = T
                           , sep = ";"
                           , stringsAsFactors = T
                           , row.names = 1
...
data_Qpcr_Ed <- read.table(file = "donnees/Detect_HLB_Hoareau_Ed_19022021_data.csv"
...
save(data_long, data_SPIR_Ed, trueP,  file = "Sauvegardes/donnees SPIR Hoarau.Rdata")
...
res.svm.36 <-   svm (y = train_svm[,paste0("qPCR_", seuil.ct)]               
                                     , x = train_svm[, grep("^X", names(train_svm))]      
...
#prédiction sur les feuilles

svm_pred_feuille <-  data.frame(predict(res.svm.36, newdata = test_svm[,grep("^X", names(test_svm))], decision.values = T))

names(svm_pred_feuille) <- c("Resultat_prediction")

svm_pred_feuille$code_ech_arbre <- factor(substr(rownames(svm_pred_feuille), 1, 2))

svm_pred_feuille$Resultat_prediction = as.numeric(as.character(svm_pred_feuille$Resultat_prediction))

svm_pred_arbres = aggregate(svm_pred_feuille$Resultat_prediction, 
                            list(svm_pred_feuille$code_ech_arbre), mean, na.rm = T)



names(svm_pred_arbres) = c("code_ech_arbre","Moyenne_resultat_arbre")


svm_pred_arbres$sd = aggregate(svm_pred_feuille$Resultat_prediction, 
                            list(svm_pred_feuille$code_ech_arbre), sd, na.rm = T)[,2]

svm_pred_arbres$statut[svm_pred_arbres$Moyenne_resultat_arbre > 0.8] = T # critère arbitraire
svm_pred_arbres$statut[svm_pred_arbres$Moyenne_resultat_arbre < 0.2] = F
svm_pred_arbres$statut[is.na(svm_pred_arbres$Moyenne_resultat_arbre)]= NA

svm_pred_arbres$statuu[svm_pred_arbres$statut == T] = "Positif"
svm_pred_arbres$statuu[svm_pred_arbres$statut == F] = "Negatif"
svm_pred_arbres$statuu[is.na(svm_pred_arbres$statut)]= "Indetermine"

# supression des éléments inutiles de la liste

rm (maliste,res.svm.36,Tirage,test_ed,test_svm,train_svm, nb.simu,nb.rep,seuil.ct,decoup,a,GCtorture)

ggplot(svm_pred_arbres)+
  geom_col(aes(y = Moyenne_resultat_arbre , x = code_ech_arbre, fill = statuu))+
  geom_errorbar(aes(y = Moyenne_resultat_arbre , x = code_ech_arbre,
                    ymin  = Moyenne_resultat_arbre - sd, 
                    ymax = Moyenne_resultat_arbre + sd,
                    fill = statuu))+
  scale_fill_viridis_d() +
  geom_hline(yintercept = 0.5, lwd = 1 , col = "red")
...
maliste <- lapply(Tirage, function(feuille){ 
  ...
  # on fait ça pour tte les feuilles mais tjrs en choisissant le nombre de rep tire aleatoirement
  code <- grep("^code_ech", names(feuille))
  qPCR <- grep("^qPCR_", names(feuille))
  sortie <- cbind.data.frame(unique(feuille[c(code,qPCR)])
                             , matrix(apply(feuille[grep("^X", names(feuille))], 2, mean)
                                      , nr = 1, dimnames = list(NULL, names(feuille)[grep("^X", names(feuille))])))
  sortie
})

test_ed <- do.call(rbind, maliste) # on recupere un spectre moyen par feuille

# ici il faut faire un sapply ou lapply pour les simus afin de comparer les methodes avec 6 pseuso replicats
...
#fct_rf <- function(nb.rep, seuil.ct, list.feuilles) {
    ...
    Pred <- grep("^X", names(train_rf), value = T)
